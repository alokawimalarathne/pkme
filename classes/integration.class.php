...
class Pickme_integration extends Generic {
	...
	function __construct() {
		...
		if (empty($_SESSION['pickme']['user_id'])) {
			//$this->guestLogin();
			return false;
		}
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['pickme']['user_id'] );
	...
	private function connect($link) {
		...
		$params = array(
			':user_id'      => $_SESSION['pickme']['user_id'],
			':session_link' => $_SESSION['pickme'][$link]
		);
	...
	private function unlink($provider) {
		...
		unset($_SESSION['pickme'][$provider]);
			...
			if(!$openid->mode) {
				...
				if ($openid->validate()) :
					...
					if( !empty($providers[$link]) ) :
						...
						$_SESSION['pickme'][$link] = $openid->identity;
						$this->connect($link);
						$_SESSION['pickme']['openIDMisc'] = $openid->getAttributes();
						$_SESSION['pickme']['openIDMisc']['type'] = $link;
						$_SESSION['pickme']['openIDMisc'][$link] = $openid->identity;
	...
	public function facebook_url() {
		...
		if( !empty($_SESSION['pickme']['facebook']) )
			return false;
		...
		if ($this->fbUser) :
		  ...
		  try {
			$_SESSION['pickme']['facebookMisc'] = $user_profile;
			$_SESSION['pickme']['facebook'] = $this->fbUser;
	...
	public function twitter_url() {
		...
		if ( !empty($_SESSION['pickme']['twitter']) )
			return false;
		...
		if( !empty($_GET['oauth_token']) ) {
			...
			$_SESSION['pickme']['twitter'] = $twitterInfo['id'];
			$_SESSION['pickme']['twitterMisc']['username'] = $twitterInfo['screen_name'];
			$_SESSION['pickme']['twitterMisc']['id'] = $twitterInfo['id'];
			$_SESSION['pickme']['twitterMisc']['name'] = $twitterInfo['name'];

		}

		return !empty($_SESSION['pickme']['twitter']) ? '#' : $this->twitterObj->getAuthenticateUrl( null,array('oauth_callback' => str_replace("www.","", SITE_PATH) . basename($_SERVER['PHP_SELF']) . '?link=twitter' ));
...
$pickme_integration = new Pickme_integration();