...
class Generic extends Connect {
	...
	public function denyAccessLogs() {
		...
		return ( ($this->getOption('profile-timestamps-admin-enable') && !in_array(1, $_SESSION['pickme']['user_level'])) || !$this->getOption('profile-timestamps-enable') );
	...
	public function generateToken() {
		...
		if(empty($_SESSION['pickme']['token']))
			$_SESSION['pickme']['token'] = md5(uniqid(mt_rand(),true));
	...
	public function valid_token($token) {
		...
		if ($_SESSION['pickme']['token'] != $token)
			return false;
