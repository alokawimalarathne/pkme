...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['pickme']['user_id'] );

		if ( empty($_SESSION['pickme']['user_id']) ) :

			$_SESSION['pickme']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isActivated() {
		...
		if ( !empty($_SESSION['pickme']['activate']) ) :
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['pickme']['restricted']) || !empty($_SESSION['pickme']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['pickme']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['pickme']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['pickme']['user_id'] ) && $level == array('*') )
			return false;
