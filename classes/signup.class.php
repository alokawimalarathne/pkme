...
class SignUp extends Generic {
	...
	public function getPost($var) {
		...
		if ( !empty($_SESSION['pickme']['openIDMisc']) ) :

			$misc = $_SESSION['pickme']['openIDMisc'];
		...
		if ( !empty($_SESSION['pickme']['facebookMisc']) ) :

			$misc = $_SESSION['pickme']['facebookMisc'];
		...
		if ( !empty($_SESSION['pickme']['twitterMisc']) ) :

			$misc = $_SESSION['pickme']['twitterMisc'];
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			$_SESSION['pickme']['user_id'] = $user_id;

			/* Social integration. */
			if ( !empty($_SESSION['pickme']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['pickme']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['pickme']['openIDMisc']) ) {
				$link = $_SESSION['pickme']['openIDMisc']['type'];
				$id = $_SESSION['pickme']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['pickme']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['pickme']['twitterMisc']['id'];
			}
