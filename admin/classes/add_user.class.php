...
class Add_user extends Generic {
	...
	function __construct() {
		...
		if(isset($_POST['add_user'])) {
			...
			$this->lname = parent::secure($_POST['lname']);
	...
	private function adduser() {
		...
		parent::query("INSERT INTO `login_users` (`user_level`, `name`, `lname`, `email`, `username`, `password`)
						VALUES (:user_level, :name, :lname, :email, :username, :password);", $params);

		$shortcodes = array(
			'site_address'	=>	SITE_PATH,
			'full_name'		=>	$this->name.' '.$this->lname,
			'username'		=>	$this->username,
			'email'			=>	$this->email,
