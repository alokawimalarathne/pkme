...
<div id="message">
	...
	<form method="post" action="page/reports.php" id="reports-date-form" class="form-inline">
		<?php _e('From:'); ?> <input name="start_date" value="<?php echo date('Y-m-d', $pickme_reports->start_date); ?>" id="from" type="date" class="hasDatePicker input-small">
		<?php _e('To:'); ?> <input name="end_date" value="<?php echo date('Y-m-d', $pickme_reports->end_date); ?>" id="to" type="date" class="hasDatePicker input-small">
	...
	<div class="span3" style="margin-left:0">
		<div class="span3 thumbnail" style="margin-left:0">
			...
			<h4><?php echo $pickme_reports->countRegisteredUsers(); ?> <small><?php _e('Registered'); ?></small></h4>
			<?php foreach ($pickme_reports->socialMethods as $method => $color) : ?>
			<h4><?php echo $pickme_reports->countSocialUsers($method); ?> <small><?php echo ucwords($method) . ' users'; ?></small></h4>
			<?php endforeach; ?>
		</div>

		<div class="span3 thumbnail" style="margin-top:20px; margin-left:0">
			<h2 style="font-weight:normal;"><?php _e('Range'); ?></h2>
			<h4><?php echo array_sum($pickme_reports->newUsers); ?> <small><?php _e('Registered'); ?></small></h4>
			<?php foreach ($pickme_reports->socialMethods as $method => $color) : ?>
			<h4><?php echo $pickme_reports->countSocialUsers($method, true); ?> <small><?php echo ucwords($method) . ' users'; ?></small></h4>
			<?php endforeach; ?>
		</div>
	...
	<div class="span3" style="margin-left:0">
		<div class="span3 thumbnail" style="margin-left:0">
			...
			<table class="table">
				...
				<tbody>
				<?php echo $pickme_reports->displayTopUsers(); ?>
				</tbody>
			</table>
		</div>
	</div>
	...
	var datasets = {
		"newUsers" : {
			...
			data: [<?php
				$counts = array();
				foreach ($pickme_reports->newUsers as $stamp => $count) $counts[] = "[$stamp, $count]";
				echo implode(',', $counts);
			?>]
			},
		<?php foreach ($pickme_reports->socialMethods as $method => $color) : ?>
		"<?php echo $method; ?>" : {
			...
			data: [<?php
				$counts = array();
				$users = $pickme_reports->socialGraph($method);
	...
	var topUsers = [
		{
			...
			data: [<?php
				$counts = array();
				$topUsers = $pickme_reports->topUsers();
